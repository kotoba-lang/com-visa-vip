(ns visa_vip.main
  "Kotodama WASM entrypoint for the visa_vip clean-room actor (L4) — Clojure port.
  ...
  facts (`visa_vip.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "visa_vip")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"visa_vip.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "visa_vip-compat" :tier tier :entities entities} 200])
...
(defn start! [] :visa_vip-compat/ready)
